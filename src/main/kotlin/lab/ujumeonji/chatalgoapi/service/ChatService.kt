...
class ChatService(
    ...
    fun processChat(
        ...
        val session =
            chatSessionRepository.findByChallengeIdAndUserId(challengeId, userId) ?: ChatSession(
    ...
    private fun generateResponse(
        ...
        try {
            ...
            return Message(
                content =
                    "죄송합니다. 현재 응답을 생성하는 중에 문제가 발생했습니다. " +
                        "잠시 후 다시 시도해 주세요.",
                sender = "system",
            )
        }
    ...
    private fun calculateUnderstandingScore(session: ChatSession): Int {
        val challenge =
            challengeService.findById(session.challengeId)
                ?: return 70
